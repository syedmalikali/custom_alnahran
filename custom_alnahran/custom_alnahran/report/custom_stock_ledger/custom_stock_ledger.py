...
def execute(filters=None):
	...
	for sle in sl_entries:
		...
		if sle.actual_qty:
			...
			sle["rate"] = sle.valuation_rate

		if sle.voucher_type in ["Sales Invoice","Purchase Invoice"]:
			rate = frappe.db.get_value(F'{sle.voucher_type} Item', sle.voucher_detail_no,['rate'])
			sle["rate"] = rate

		if sle.voucher_type in ["Stock Entry"]:
			rate = frappe.db.get_value("Stock Entry Detail", sle.voucher_detail_no,['basic_rate'])
			sle["rate"] = rate
